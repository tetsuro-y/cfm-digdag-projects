...
# ワークフローの実行
echo "digdag start ${PJ_NAME} ${PJ_NAME} --session now --params-file ${PJ_DIR}/config.yml --endpoint ${DIGDAG_SERVER} -p exec_env=${EXECUTE_ENV} -p exec_server=${DIGDAG_SERVER}"
digdag start ${PJ_NAME} ${PJ_NAME} --session now --params-file ${PJ_DIR}/config.yml --endpoint ${DIGDAG_SERVER} -p exec_env=${EXECUTE_ENV} -p exec_server=${DIGDAG_SERVER}
