...
INNER JOIN TGOODSDETAIL ON FLGOODSDETAILID = GDGOODSDETAILID
WHERE
    ROWNUM = 1
;
...
(
    ...
    TRI_BUYGOODSID VARCHAR(30),
    TRI_BUYPRICE INTEGER,
    TRI_FAVGOODSID VARCHAR(30)
)
DISTRIBUTE ON (TRI_UNIQUEGOODSID)
;
...
FROM (
    ...
    SELECT
        ...
        ,GDGOODSID AS SRM_GOODSID
        ,SBM_OFFERID || '_' || GDGOODSID AS SRM_UNIQUEGOODSID
        ,SBM_AREATYPEID AS SRM_AREATYPEID
        ,LEAD(SBM_OPENDT) OVER (PARTITION BY SBM_CAMPAIGNID, SBM_DEVICEID, SBM_MEMBERID, SBM_AREATYPEID, GDGOODSID ORDER BY SBM_OPENDT) AS SRM_LEADOPENDT
    FROM (
        SELECT DISTINCT
...
FROM (
    ...
    SELECT
        ...
        ,GDGOODSID AS SRL_GOODSID
        ,SBL_OFFERID || '_' || GDGOODSID AS SRL_UNIQUEGOODSID
        ,LEAD(SBL_OFFERDELIVERYDT) OVER (PARTITION BY SBL_CAMPAIGNID, SBL_DEVICEID, SBL_MEMBERID, GDGOODSID ORDER BY SBL_OFFERDELIVERYDT) AS SRL_LEADDELIVERYDT
    FROM (
        SELECT DISTINCT
...
FROM (
    SELECT
        ...
        ,COUNT(DISTINCT CASE WHEN TD_OPENDT < TD_SENDDT::DATE + INTERVAL '8DAYS' AND TD_CHANNELID = 1 AND TD_DEVICEID = 2 THEN NVL(TD_OFFERID, TD_EMAILID) ELSE NULL END) AS DD_CNT_OPEN--配信から7日以内の開封実績対象
        ,COUNT(DISTINCT CASE WHEN TD_CLICKDT < TD_SENDDT::DATE + INTERVAL '8DAYS' THEN TD_OFFERID ELSE NULL END) AS DD_CNT_CLICK--配信から7日以内の流入実績対象
    FROM
        TAT_DB_RESULT_PERSONALIZE_TMP_DELIVERY
    GROUP BY
        DD_SENDDT
    ...
    LEFT OUTER JOIN (
        ...
        WHERE
            ...
            AND HVU_OFFERID IS NOT NULL--不正なパラメータのレコードを除く
        GROUP BY
            VD_SENDDT
            ,VD_CAMPAIGNID
            ,VD_CHANNELID
...
FROM
    ...
    LEFT OUTER JOIN (
        SELECT
            ...
            ,COUNT(DISTINCT TRI_BUYGOODSID) AS TRI_CNT_BUY_SENDITEM--購買商品数（1つの配信商品に対しそれが買われたかどうかの数を数えるのでDISTINCTをとる）
            ,SUM(TRI_BUYPRICE) AS TRI_REVENUE_BUY_SENDITEM--購買金額（金額は実績として複数購買の場合も全部対象とするのですべてSUMする）
            ,COUNT(DISTINCT TRI_FAVGOODSID) AS TRI_CNT_FAVORITE_ITEM--お気に入り登録商品数
        FROM
...
FROM
    ...
    LEFT OUTER JOIN (
        SELECT
            ...
            ,COUNT(DISTINCT CASE WHEN TD_OPENDT < TD_SENDDT::DATE + INTERVAL '8DAYS' AND TD_CHANNELID = 1 AND TD_DEVICEID = 2 THEN NVL(TD_OFFERID, TD_EMAILID) ELSE NULL END) AS CNT_OPEN
        FROM
            TAT_DB_RESULT_PERSONALIZE_TMP_DELIVERY
        GROUP BY
            TD_CAMPAIGNID
...
FROM
    ...
    LEFT OUTER JOIN (
        SELECT
            ...
            ,COUNT(DISTINCT CASE WHEN TD_CLICKDT < TD_SENDDT::DATE + INTERVAL '8DAYS' THEN TD_OFFERID ELSE NULL END) AS CNT_CLICK
        FROM
            TAT_DB_RESULT_PERSONALIZE_TMP_DELIVERY
        GROUP BY
            TD_CAMPAIGNID
...
FROM
    ...
    SELECT
        ...
        ,TRI_DEVICEID
        ,TRI_HOUR_ORDER
        ,COUNT(DISTINCT TRI_BUYGOODSID) AS CNT_BUYITEM
    FROM
        TAT_DB_RESULT_PERSONALIZE_TMP_RESULT_ITEM
    GROUP BY
        TRI_CAMPAIGNID
