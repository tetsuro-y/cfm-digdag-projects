...
SELECT
	...
	,CNT_SEND_MO
	,CNT_OPEN
	,ROUND(CAST(CNT_OPEN AS NUMERIC) / CAST(CNT_SEND_TOTAL AS NUMERIC), 2) AS CNT_OPEN_PER		--開封率_PC
	,CNT_CLICK_TOTAL
	,CNT_CLICK_PC
	,CNT_CLICK_MO
	,ROUND(CAST(CNT_CLICK_PC AS NUMERIC) / CAST(CNT_OPEN AS NUMERIC), 2) AS OPEN_CLICK_PER_PC		--開封流入率_PC
	,ROUND(CAST(CNT_CLICK_MO AS NUMERIC) / CAST(CNT_SEND_MO AS NUMERIC), 2) AS SEND_CLICK_PER_MO	--配信流入率_MO
	,REVENUE_TOTAL
	,REVENUE_PC
	,REVENUE_MO
FROM (
	SELECT
		...
		,MASSMAIL_SENDDT			--配信日
		,50000 AS CNT_SEND_TOTAL	--配信数_全体
		,30000 AS CNT_SEND_PC		--配信数_PC
		,20000 AS CNT_SEND_MO		--配信数_MO
		,5000 AS CNT_OPEN			--開封数
