...
/* 流入数、CV数の集計 */
/* CAMPAIGNID, CHANNEL_DETAILIDごとの流入数、CV数を集計 */
...
DISTRIBUTE ON (TPCC_SENDDT, TPCC_OSID, TPCC_CAMPAIGNID, TPCC_CHANNEL_DETAILID)
;
...
FROM (
    SELECT 
        HVU_SENDDT AS CC_SENDDT
        ,HVU_OSID AS CC_OSID
        ,HVU_CAMPAIGNID AS CC_CAMPAIGNID
        ,COUNT(HVU_FULLVISITORID) AS CC_CNT_CLICK
        ,COUNT(HVU_FULLVISITORID_CV) AS CC_CNT_CV
...
INNER JOIN (
    /* マスの配信数をキャンペーン単位で割るために、対象日付のキャンペーン数を取得 */
    SELECT
    ...
    ON CC_SENDDT = CCN_SENDDT
    AND CC_CHANNEL_DETAILID = CCN_CHANNEL_DETAILID
;

/* マス・新着・パーソナライズPUSH 配信数,流入数, CV数, 経由売上 */
...
FROM (
    SELECT
        BA_SENDDT AS PSD_SENDDT
        ,BA_OSID AS PSD_OSID
        ,BA_PUSHTYPECATEGORYID AS PSD_PUSHTYPECATEGORYID
        ,BA_CAMPAIGNID AS PSD_CAMPAIGNID
        ,SUM(BA_CNT_SEND) AS PSD_CNT_SEND
    FROM (
        /* PUSHTYPECATEGORYID=1,7で、CAMPAIGNIDの取得方法が異なるためUNIONで取得 */
        SELECT
            TPSP_SENDDT AS BA_SENDDT
            ,TPSP_OSID AS BA_OSID
            ,TPSP_PUSHTYPECATEGORYID AS BA_PUSHTYPECATEGORYID
            ,CAG_CAMPAIGNID AS BA_CAMPAIGNID
            ,TPSP_CNT_SEND AS BA_CNT_SEND
        FROM
            TAT_DB_RESULT_TMP_PUSH_SEND_PUSHTYPE
            INNER JOIN (
                ...
                FROM
                    ...
                    INNER JOIN TPNCAMPAIGN ON PNCCAMPAIGNID = PNDCAMPAIGNID /* マスタに存在しないCAMPAIGNIDの除外 */
                WHERE
        ...
        SELECT
            ...
            ,TPSP_OSID AS BA_OSID
            ,TPSP_PUSHTYPECATEGORYID AS BA_PUSHTYPECATEGORYID
            ,TPSP_ITEMID AS BA_CAMPAIGNID     /* PUSHTYPECATEGORYID=7はITEMIDがCAMPAIGNID */
            ,TPSP_CNT_SEND AS BA_CNT_SEND
        FROM
            TAT_DB_RESULT_TMP_PUSH_SEND_PUSHTYPE
            INNER JOIN TPNCAMPAIGN ON PNCCAMPAIGNID = TPSP_ITEMID /* マスタに存在しないCAMPAIGNIDの除外 */
        WHERE
...
LEFT JOIN (
    SELECT
        DATE_PART('HOUR', DM_CONTACTDT) AS HD_HOUR
        ,DM_PUSHTYPECATEGORYID AS HD_PUSHTYPECATEGORYID
        ,DM_DEVICETYPEID AS HD_DEVICETYPEID
...
FROM (
    SELECT
        BA_HOUR AS PHS_HOUR
        ,BA_DEVICETYPEID AS PHS_DEVICETYPEID
        ,BA_CAMPAIGNID AS PHS_CAMPAIGNID
        ,SUM(BA_CNT_SEND) AS PHS_CNT_SEND
    FROM (
        /* PUSHTYPECATEGORYID=1,7で、CAMPAIGNIDの取得方法が異なるためUNIONで取得 */
        SELECT
            HST_HOUR AS BA_HOUR
            ,HST_DEVICETYPEID AS BA_DEVICETYPEID
            ,CAG_CAMPAIGNID AS BA_CAMPAIGNID
            ,HST_CNT_SEND AS BA_CNT_SEND
        FROM
            TAT_DB_RESULT_HOURLY_SEND_TMP
            INNER JOIN (
                /* TPNNOTICEDELIVERYからCAMPAIGNIDの抽出 */
        ...
        SELECT
            HST_HOUR AS BA_HOUR
            ,HST_DEVICETYPEID AS BA_DEVICETYPEID
            ,HST_ITEMID AS BA_CAMPAIGNID    /* PUSHTYPECATEGORYID=7はITEMIDがCAMPAIGNID */
            ,HST_CNT_SEND AS BA_CNT_SEND
        FROM
            TAT_DB_RESULT_HOURLY_SEND_TMP
            INNER JOIN TPNCAMPAIGN ON PNCCAMPAIGNID = HST_ITEMID /* マスタに存在しないCAMPAIGNIDの除外 */
        WHERE
            HST_PUSHTYPECATEGORYID = 7 /* セグメント */
    ) AS PERSONALIZE_BASE
    GROUP BY
      PHS_HOUR
      ,PHS_DEVICETYPEID
...
SELECT 
    ...
    ,(PSI_CNT_CV + PSA_CNT_CV) AS RAP_CNT_CV_ALL
    ,PSI_CNT_CV AS RAP_CNT_CV_IOS
    ,PSA_CNT_CV AS RAP_CNT_CV_ANDROID
    ,ROUND(100.0 * (CASE WHEN RAP_CNT_SEND_IOS > 0 THEN RAP_CNT_CLICK_IOS / RAP_CNT_SEND_IOS ELSE 0 END), 2) AS RAP_RATE_CLICK_PER_SEND_IOS
    ,ROUND(100.0 * (CASE WHEN RAP_CNT_SEND_ANDROID > 0 THEN RAP_CNT_CLICK_ANDROID / RAP_CNT_SEND_ANDROID ELSE 0 END), 2) AS RAP_RATE_CLICK_PER_SEND_ANDROID
    ,ROUND(100.0 * (CASE WHEN RAP_CNT_CLICK_IOS > 0 THEN RAP_CNT_CV_IOS / RAP_CNT_CLICK_IOS ELSE 0 END), 2) AS RAP_RATE_CV_PER_CLICK_IOS
    ,ROUND(100.0 * (CASE WHEN RAP_CNT_CLICK_ANDROID > 0 THEN RAP_CNT_CV_ANDROID / RAP_CNT_CLICK_ANDROID ELSE 0 END), 2) AS RAP_RATE_CV_PER_CLICK_ANDROID
