...
--日次実績更新
DELETE FROM TAT_DB_RESULT_NEWARRIVAL
WHERE RNA_SENDDT >= '${pd_base_date}'::DATE + INTERVAL '-8DAYS' OR RNA_SENDDT < DATE_TRUNC('MONTH','${pd_base_date}'::DATE + INTERVAL '-25MONTHS')
;
...
SELECT
    ...
    ,RNA_CAMPAIGNID
    --配信数
    ,(RNA_CNT_SEND_PC_HEAVY + RNA_CNT_SEND_PC_LIGHT + RNA_CNT_SEND_MO) AS RNA_CNT_SEND_ALL
    ,(RNA_CNT_SEND_PC_HEAVY + RNA_CNT_SEND_PC_LIGHT) AS RNA_CNT_SEND_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_SEND_PC_HEAVY END AS RNA_CNT_SEND_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_SEND_PC_LIGHT END AS RNA_CNT_SEND_PC_LIGHT
    ,RNA_CNT_SEND_MO
    --開封数
    ,(RNA_CNT_OPEN_PC_HEAVY + RNA_CNT_OPEN_PC_LIGHT) AS RNA_CNT_OPEN_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_OPEN_PC_HEAVY END AS RNA_CNT_OPEN_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_OPEN_PC_LIGHT END AS RNA_CNT_OPEN_PC_LIGHT
    --開封率
    ,RNA_RATE_OPEN_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_RATE_OPEN_PC_HEAVY END AS RNA_RATE_OPEN_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_RATE_OPEN_PC_LIGHT END AS RNA_RATE_OPEN_PC_LIGHT
    --流入数
    ,(RNA_CNT_CLICK_PC_HEAVY + RNA_CNT_CLICK_PC_LIGHT) AS RNA_CNT_CLICK_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_CLICK_PC_HEAVY END AS RNA_CNT_CLICK_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_CNT_CLICK_PC_LIGHT END AS RNA_CNT_CLICK_PC_LIGHT
    ,RNA_CNT_CLICK_MO
    --開封流入率
    ,RNA_RATE_CLICK_PER_OPEN_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_RATE_CLICK_PER_OPEN_PC_HEAVY END AS RNA_RATE_CLICK_PER_OPEN_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_RATE_CLICK_PER_OPEN_PC_LIGHT END AS RNA_RATE_CLICK_PER_OPEN_PC_LIGHT
    --配信流入率
    ,RNA_RATE_CLICK_PER_SEND_MO
    --経由売上
    ,(RNA_REVENUE_TOTAL_PC_HEAVY + RNA_REVENUE_TOTAL_PC_LIGHT) AS RNA_REVENUE_TOTAL_PC_ALL
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_REVENUE_TOTAL_PC_HEAVY END AS RNA_REVENUE_TOTAL_PC_HEAVY
    ,CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE RNA_REVENUE_TOTAL_PC_LIGHT END AS RNA_REVENUE_TOTAL_PC_LIGHT
    ,RNA_REVENUE_TOTAL_MO
    --掲載アイテム数
    ,NVL(TRI_CNT_SENDITEM_PC_ALL, 0) AS RNA_CNT_SENDITEM_PC_ALL
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_SENDITEM_PC_HEAVY END), 0) AS RNA_CNT_SENDITEM_PC_HEAVY
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_SENDITEM_PC_LIGHT END), 0) AS RNA_CNT_SENDITEM_PC_LIGHT
    --掲載アイテム購買人数
    ,NVL(TRI_CNT_BUYMEMBER_SENDITEM_PC_ALL, 0) AS RNA_CNT_BUYMEMBER_SENDITEM_PC_ALL
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_BUYMEMBER_SENDITEM_PC_HEAVY END), 0) AS RNA_CNT_BUYMEMBER_SENDITEM_PC_HEAVY
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_BUYMEMBER_SENDITEM_PC_LIGHT END), 0) AS RNA_CNT_BUYMEMBER_SENDITEM_PC_LIGHT
    --掲載アイテム購買数
    ,NVL(TRI_CNT_BUY_SENDITEM_PC_ALL, 0) AS RNA_CNT_BUY_SENDITEM_PC_ALL
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_BUY_SENDITEM_PC_HEAVY END), 0) AS RNA_CNT_BUY_SENDITEM_PC_HEAVY
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_BUY_SENDITEM_PC_LIGHT END), 0) AS RNA_CNT_BUY_SENDITEM_PC_LIGHT
    --掲載アイテム売上
    ,NVL(TRI_REVENUE_BUY_SENDITEM_PC_ALL, 0) AS RNA_REVENUE_BUY_SENDITEM_PC_ALL
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_REVENUE_BUY_SENDITEM_PC_HEAVY END), 0) AS RNA_REVENUE_BUY_SENDITEM_PC_HEAVY
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_REVENUE_BUY_SENDITEM_PC_LIGHT END), 0) AS RNA_REVENUE_BUY_SENDITEM_PC_LIGHT
    --掲載アイテムお気に入り登録
    ,NVL(TRI_CNT_FAVORITE_ITEM_PC_ALL, 0) AS RNA_CNT_FAVORITE_ITEM_PC_ALL
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_FAVORITE_ITEM_PC_HEAVY END), 0) AS RNA_CNT_FAVORITE_ITEM_PC_HEAVY
    ,NVL((CASE WHEN RNA_CAMPAIGNID = 9010 THEN 0 ELSE TRI_CNT_FAVORITE_ITEM_PC_LIGHT END), 0) AS RNA_CNT_FAVORITE_ITEM_PC_LIGHT
FROM (
    SELECT
        ...
        --開封流入率（PC）
        ,CASE WHEN RNA_CNT_SEND_PC_HEAVY > 0
            THEN ROUND((RNA_CNT_OPEN_PC_HEAVY * 1.0) / (RNA_CNT_SEND_PC_HEAVY * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_OPEN_PC_HEAVY
        ,CASE WHEN RNA_CNT_SEND_PC_LIGHT > 0
            THEN ROUND((RNA_CNT_OPEN_PC_LIGHT * 1.0) / (RNA_CNT_SEND_PC_LIGHT * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_OPEN_PC_LIGHT
        ,CASE WHEN RNA_CNT_SEND_PC_HEAVY + RNA_CNT_SEND_PC_LIGHT > 0
            THEN ROUND(((RNA_CNT_OPEN_PC_HEAVY + RNA_CNT_OPEN_PC_LIGHT) * 1.0) / ((RNA_CNT_SEND_PC_HEAVY + RNA_CNT_SEND_PC_LIGHT) * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_OPEN_PC_ALL
        --クリック数
        ...
        --開封流入率
        ,CASE WHEN RNA_CNT_OPEN_PC_HEAVY > 0
            THEN ROUND((RNA_CNT_CLICK_PC_HEAVY * 1.0) / (RNA_CNT_OPEN_PC_HEAVY * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_CLICK_PER_OPEN_PC_HEAVY
        ,CASE WHEN RNA_CNT_OPEN_PC_LIGHT > 0
            THEN ROUND((RNA_CNT_CLICK_PC_LIGHT * 1.0) / (RNA_CNT_OPEN_PC_LIGHT * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_CLICK_PER_OPEN_PC_LIGHT
        ,CASE WHEN (RNA_CNT_OPEN_PC_HEAVY + RNA_CNT_OPEN_PC_LIGHT) > 0
            THEN ROUND(((RNA_CNT_CLICK_PC_HEAVY + RNA_CNT_CLICK_PC_LIGHT) * 1.0) / ((RNA_CNT_OPEN_PC_HEAVY + RNA_CNT_OPEN_PC_LIGHT) * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_CLICK_PER_OPEN_PC_ALL
        --配信流入率
        ,CASE WHEN RNA_CNT_SEND_MO > 0
            THEN ROUND((RNA_CNT_CLICK_MO * 1.0) / (RNA_CNT_SEND_MO * 1.0), 2)
            ELSE 0.00
        END AS RNA_RATE_CLICK_PER_SEND_MO
        --経由売上
    ...
    LEFT OUTER JOIN (
        SELECT
            ...
            ,TRI_CNT_SENDITEM_PC_HEAVY + TRI_CNT_SENDITEM_PC_LIGHT AS TRI_CNT_SENDITEM_PC_ALL
            --掲載アイテム購買人数
            ,COUNT(DISTINCT CASE WHEN TRI_DEVICEID = 2 AND TRI_USERTYPEID = 1 THEN TRI_MEMBERID ELSE NULL END) AS TRI_CNT_BUYMEMBER_SENDITEM_PC_HEAVY
            ,COUNT(DISTINCT CASE WHEN TRI_DEVICEID = 2 AND TRI_USERTYPEID = 2 THEN TRI_MEMBERID ELSE NULL END) AS TRI_CNT_BUYMEMBER_SENDITEM_PC_LIGHT
            ,TRI_CNT_BUYMEMBER_SENDITEM_PC_HEAVY + TRI_CNT_BUYMEMBER_SENDITEM_PC_LIGHT AS TRI_CNT_BUYMEMBER_SENDITEM_PC_ALL
            --掲載アイテム購買数
            ...
            ,TRI_CNT_BUY_SENDITEM_PC_HEAVY + TRI_CNT_BUY_SENDITEM_PC_LIGHT AS TRI_CNT_BUY_SENDITEM_PC_ALL
            --掲載アイテム購買売上
            ...
            ,TRI_REVENUE_BUY_SENDITEM_PC_HEAVY + TRI_REVENUE_BUY_SENDITEM_PC_LIGHT AS TRI_REVENUE_BUY_SENDITEM_PC_ALL
            --掲載アイテムお気に入り登録数
            ...
            ,TRI_CNT_FAVORITE_ITEM_PC_HEAVY + TRI_CNT_FAVORITE_ITEM_PC_LIGHT AS TRI_CNT_FAVORITE_ITEM_PC_ALL
        FROM
