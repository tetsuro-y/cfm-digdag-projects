...
FROM
    ...
    SELECT
        ...
        ,LEFT(PW_SENDDT, 8) AS HVU_SENDDT
    ...
    WHERE
        DATEDIFF(PW_DATE, LEFT(PW_SENDDT, 8)) >= 0
        AND DATEDIFF(PW_DATE, LEFT(PW_SENDDT, 8)) <= 7--配信から7日以内の流入に絞る
    ...
    SELECT
        ...
        ,LEFT(PP_SENDDT, 8) AS HVU_SENDDT
        ,3 AS HVU_CHANNELID--PUSH
    ...
    FROM
        ...
        WHERE
            REGEXP_MATCH(HITS.APPINFO.SCREENNAME, R'^.*push_type=PUSH_P'))
    WHERE
        DATEDIFF(PP_DATE, LEFT(PP_SENDDT, 8)) >= 0
        AND DATEDIFF(PP_DATE, LEFT(PP_SENDDT, 8)) <= 7--配信から7日以内の流入に絞る
