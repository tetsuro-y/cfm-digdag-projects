...
FROM
    ...
    SELECT
        ...
        ,CASE
            WHEN UPPER(MM_DEVICE) = 'MO' THEN 1
            WHEN UPPER(MM_DEVICE) = 'PC' THEN 2
        END AS VC_DEVICE
        ,CASE
            WHEN UPPER(MM_DEVICE) = 'PC'
                 AND LENGTH(MM_CAMPAIGN) >= 12--YYYYMMDD_PC_以降にEMAILIDが入っているキャンペーンに絞る
    ...
    FROM (
        SELECT
            ...
            ,NTH(2, SPLIT(TRAFFICSOURCE.CAMPAIGN, '_')) AS MM_DEVICE--この段階でUPPERをかけるとエラーになる
