...
# 繰り返しループする回数を決定する
if [ $(( ${file_length} % ${SEGMENT_NUM} )) -eq 0 ]; then
    loop=$(( ${file_length} / ${SEGMENT_NUM} ))
else
    loop=$(( ${file_length} / ${SEGMENT_NUM} + 1 ))
fi
...
for i in $(seq 1 ${loop}); do
    # ループの開始と終了ポジジョン
    echo $(( (${i} - 1) * ${SEGMENT_NUM} + 1 )),$(( ${i} * ${SEGMENT_NUM} ))

    # 該当ファイルを取得する
    files=$(cat ${listfile} | sed -n $(( (${i} - 1) * ${SEGMENT_NUM} + 1 )),$(( (${i}) * ${SEGMENT_NUM} ))p)
    echo $files
