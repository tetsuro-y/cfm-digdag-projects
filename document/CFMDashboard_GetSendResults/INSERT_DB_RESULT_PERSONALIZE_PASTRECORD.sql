...
CREATE TEMP TABLE TAT_DB_RESULT_PERSONALIZE_TMP_TERM AS
SELECT
    '20151101' AS TT_STARTDT
    ,'20161101' AS TT_ENDDT
;
...
FROM
    TRTM_OFFER_DELIVERY
WHERE
    OFFER_DELIVERY_DT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
    AND OFFER_DELIVERY_DT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
...
WHERE
    DELIVERYDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
    AND DELIVERYDT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
...
SELECT DISTINCT
    ...
    ,NULL AS TD_OFFERID
    ,MPM_MAPPINGID AS TD_CAMPAIGNID
    ,UMDMEMBERID AS TD_MEMBERID
    ,UMDEMAILID AS TD_EMAILID
    ,1 AS TD_CHANNELID--メール
    ,CASE
        WHEN UMDCHANNELCODE = 1 THEN 1
        ELSE 2--PC/SP
    END AS TD_DEVICEID
    ,UMDCONTACTDT::DATE AS TD_SENDDT
    ,CASE
        WHEN UMDCHANNELCODE <> 1 AND UMDOPENFLAG IS NOT NULL THEN UMDCONTACTDT::DATE
        ELSE NULL
    END AS TD_OPENDT
    ,NULL AS TD_CLICKDT
	,NULL AS TD_CVDT
FROM
    TUCEMAILDELIVERY
    INNER JOIN TAT_DB_MASTER_PARAMETER_MAPPING ON UMDCAMPAIGNCODE = MPM_PARAMETER
    INNER JOIN TUCCAMPAIGN ON UMDCAMPAIGNCODE = UCCAMPAIGNCODE
    INNER JOIN TUCOFFER ON UOOFFERCODE = UMDOFFERCODE
WHERE
    UMDCONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
    AND UMDCONTACTDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
    AND UMDCAMPAIGNCODE NOT IN ('C000000335','C000000167','C000000438') --新着、早稲田除く※TAT_DB_MASTER_PARAMETER_MAPPINGを使うので対象外のキャンペーンは除かれるがデータ量を減らすため
...
SELECT DISTINCT
    ...
    ,NULL AS TD_OFFERID
    ,MPM_MAPPINGID AS TD_CAMPAIGNID
    ,PNDMEMBERID AS TD_MEMBERID
    ,NULL AS TD_EMAILID
    ,2 AS TD_CHANNELID--LINE
    ,2 AS TD_DEVICEID--SP
    ,PNDCONTACTDT AS TD_SENDDT
    ,NULL AS TD_OPENDT
    ,NULL AS TD_CLICKDT
    ,NULL AS TD_CVDT
FROM (
    SELECT
        ...
        ,PNDCONTACTDT
    FROM
        VPNNOTICEDELIVERY4LINE
    WHERE
        PNDCONTACTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
        AND PNDCONTACTDT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)

    UNION ALL

    SELECT
        PNDLCAMPAIGNID AS PNDCAMPAIGNID
        ,PNDLMEMBERID AS PNDMEMBERID
        ,PNDLCONTACTDT AS PNDCONTACTDT
    FROM
        TPNNOTICEDELIVERY4LINE
    WHERE
        PNDLCONTACTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
        AND PNDLCONTACTDT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
) AS LINEBASE
INNER JOIN TAT_DB_MASTER_PARAMETER_MAPPING ON PNDCAMPAIGNID::VARCHAR(30) = MPM_PARAMETER
;
...
WHERE
    ...
    AND ORORDERDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
GROUP BY
    TO_ORDERDT
    ,TO_MEMBERID
    ,TO_GOODSID
    ...
    FROM (
        ...
        FROM (
            ...
            FROM
                TFAVORITELIST
            WHERE
                FLREGISTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
            ...
            WHERE
                FLDREGISTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
...
FROM (
    SELECT
        ...
        ,COUNT(DISTINCT NVL(TD_OFFERID, NVL(TD_EMAILID, TD_MEMBERID))) AS DD_CNT_SEND
        ...
        ,COUNT(DISTINCT CASE WHEN TD_CVDT < TD_CVDT::DATE + INTERVAL '8DAYS' THEN TD_OFFERID ELSE NULL END) AS DD_CNT_CV
    FROM
        TAT_DB_RESULT_PERSONALIZE_TMP_DELIVERY
    GROUP BY
        DD_SENDDT
    ...
    LEFT OUTER JOIN (
        ...
        WHERE
            HVU_SENDDT >= (SELECT TT_STARTDT::DATE FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
            AND HVU_SENDDT < (SELECT TT_ENDDT::DATE FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
            ...
            AND HVU_VISITTIME < HVU_SENDDT::DATE + INTERVAL '8DAYS'--配信から7日以内の流入に限定
        GROUP BY
            VD_SENDDT
            ,VD_CAMPAIGNID
            ,VD_CHANNELID
...
WHERE
    RPA_SENDDT >= (SELECT TT_STARTDT::DATE FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
    AND RPA_SENDDT < (SELECT TT_ENDDT::DATE FROM TAT_DB_RESULT_PERSONALIZE_TMP_TERM)
;
