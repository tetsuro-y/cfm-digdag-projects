...
WHERE
    (
        (
            RML_SENDDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_MASSMAIL_TMP_TERM)
            AND
            RML_SENDDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_MASSMAIL_TMP_TERM)
        )
    ...
    (
        (
            RML_VISITDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_MASSMAIL_TMP_TERM)
            AND
            RML_VISITDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_MASSMAIL_TMP_TERM)
        )
