...
CREATE TEMP TABLE TAT_DB_RESULT_TRANSACTION_TMP_TERM AS
SELECT
    '20160201' AS TT_STARTDT
    ,'20180301' AS TT_ENDDT
;
...
WHERE
    RTR_SENDDT >= (SELECT TT_STARTDT::DATE FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
    AND RTR_SENDDT < (SELECT TT_ENDDT::DATE FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
;
...
FROM (
    ...
    FROM (
        ...
        WHERE
            TMDCONTACTDT::DATE >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
            AND TMDCONTACTDT::DATE < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
            AND TMDMALLID = 1
    ...
    LEFT OUTER JOIN (
        ...
        WHERE
            HVU_SENDDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
            AND HVU_SENDDT <= (SELECT TT_ENDDT::DATE FROM TAT_DB_RESULT_TRANSACTION_TMP_TERM)
            AND HVU_CHANNELID = 1--メール
            AND HVU_CHANNEL_DETAILID = 3--トランザクション
            AND HVU_VISITTIME <= HVU_SENDDT + INTERVAL '8DAYS'
