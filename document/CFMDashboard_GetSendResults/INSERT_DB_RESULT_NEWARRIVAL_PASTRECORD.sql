...
CREATE TEMP TABLE TAT_DB_RESULT_NEWARRIVAL_TMP_TERM AS
SELECT
    '2016/03/01'::DATE AS TT_STARTDT
    ,'2016/04/01'::DATE AS TT_ENDDT
;
...
SELECT
    ...
    ,TD_SENDDT
    ,TD_OPENDT
    ,NULL::TIMESTAMP AS TD_CLICKDT
FROM (
    --旧新着メール（~20170129）
    SELECT
        UMD_CAMPAIGNID AS TD_CAMPAIGNID
        ,UMD_MEMBERID AS TD_MEMBERID
        ,UMD_EMAILID AS TD_EMAILID
        ,UMD_DEVICEID AS TD_DEVICEID
        ,CASE
            WHEN UMD_CAMPAIGNID = 9000 AND UMD_DEVICEID = 2 THEN SLM_USERTYPEID--TOWNの場合ヘビー/ライト区分を使用
            WHEN UMD_CAMPAIGNID = 9010 AND UMD_DEVICEID = 2 THEN 1--USEDの場合PCは全員ヘビー区分
            ELSE 99
        END AS TD_USERTYPEID
        ,UMD_SENDDT AS TD_SENDDT
        ,UMD_OPENDT AS TD_OPENDT
    FROM (
        SELECT
            CASE 
                WHEN UMDCAMPAIGNCODE = 'C000000167' THEN 9000
                WHEN UMDCAMPAIGNCODE = 'C000000335' THEN 9010
            END::SMALLINT AS UMD_CAMPAIGNID
            ,UMDMEMBERID AS UMD_MEMBERID
            ,UMDEMAILID AS UMD_EMAILID
            ,CASE WHEN UMDCHANNELCODE = 1 THEN 1 ELSE 2 END::BYTEINT AS UMD_DEVICEID
            ,UMDCONTACTDT::DATE AS UMD_SENDDT
            ,CASE
                WHEN UMDCHANNELCODE <> 1 AND UMDOPENFLAG IS NOT NULL THEN UMDCONTACTDT
                ELSE NULL
            END::TIMESTAMP AS UMD_OPENDT
        FROM
            TUCEMAILDELIVERY
        WHERE
            UMDCONTACTDT >= (SELECT TO_CHAR(TT_STARTDT, 'YYYYMMDD') FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            AND UMDCONTACTDT < (SELECT TO_CHAR(TT_ENDDT, 'YYYYMMDD') FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            AND UMDCONTACTDT < '20170130'
            AND UMDCAMPAIGNCODE IN ('C000000167', 'C000000335')--TOWN/USED
        ) AS UMD
        INNER JOIN (
            SELECT
                9000::SMALLINT AS SLM_CAMPAIGNID
                ,CONTACTDT AS SLM_CONTACTDT
                ,EMAILID AS SLM_EMAILID
                ,USERTYPE AS SLM_USERTYPEID
            FROM
                TUCNEWARRIVAL_TOWN_SENDLIST_MEMBER
            WHERE
                CONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
                AND CONTACTDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)

            UNION ALL

            SELECT
                9010::SMALLINT AS SLM_CAMPAIGNID
                ,CONTACTDT AS SLM_CONTACTDT
                ,EMAILID AS SLM_EMAILID
                ,USERTYPE AS SLM_USERTYPEID
            FROM
            ...
            WHERE
                CONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
                AND CONTACTDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        ) AS SLM ON UMD_SENDDT = SLM_CONTACTDT AND UMD_CAMPAIGNID = SLM_CAMPAIGNID AND UMD_EMAILID = SLM_EMAILID
    ...
    SELECT
        ...
        ,CASE 
            WHEN SL_CAMPAIGNID = 9000 AND MMD.MOBILEFLAG = 0 THEN SL_USERTYPEID
            WHEN SL_CAMPAIGNID = 9010 AND MMD.MOBILEFLAG = 0 THEN 1
            ELSE 99
        END AS TD_USERTYPEID
    ...
    FROM
        ...
        INNER JOIN (
            SELECT
                9000 AS SL_CAMPAIGNID
                ,CONTACTDT AS SL_SENDDT
                ,EMAILID AS SL_EMAILID
                ,USERTYPE AS SL_USERTYPEID
            FROM
                TUCNEWARRIVAL_TOWN_SENDLIST_MEMBER
            WHERE
                CONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            ...
            WHERE
                CONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
                AND CONTACTDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        ) AS SL ON MMD.MAILMAGCAMPAIGNID = SL_CAMPAIGNID AND MMD.DELIVERYDT::DATE = SL_SENDDT AND MMDD.EMAILID = SL_EMAILID
    ...
    WHERE
        DELIVERYDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        AND DELIVERYDT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        AND MPM_CHANNELID = 1
...
FROM (
    ...
    WHERE
        CONTACTDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        AND CONTACTDT < (SELECT TT_ENDDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
) AS TSG
INNER JOIN (
    SELECT
        TD_CAMPAIGNID
...
WHERE
    ...
    AND ORORDERDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
    AND ORORDERDT < (SELECT TT_ENDDT::TIMESTAMP + INTERVAL '8DAYS' FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
GROUP BY
    TO_ORDERDT
    ,TO_MEMBERID
    ,TO_GOODSID
...
FROM (
    ...
    FROM (
        ...
        FROM
            TFAVORITELIST
        WHERE
            FLREGISTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            AND FLREGISTDT < (SELECT TT_ENDDT::TIMESTAMP + INTERVAL '8DAYS' FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
        ...
        WHERE
            FLDREGISTDT >= (SELECT TT_STARTDT::TIMESTAMP FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            AND FLDREGISTDT < (SELECT TT_ENDDT::TIMESTAMP + INTERVAL '8DAYS' FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
...
FROM (
    ...
    LEFT OUTER JOIN (
        ...
        WHERE
            HVU_SENDDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
            AND HVU_SENDDT < (SELECT TT_ENDDT + INTERVAL '8DAYS' FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
...
WHERE 
    RNA_SENDDT >= (SELECT TT_STARTDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
    AND RNA_SENDDT < (SELECT TT_ENDDT FROM TAT_DB_RESULT_NEWARRIVAL_TMP_TERM)
;
